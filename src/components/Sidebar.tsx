...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  useEffect(() => {
    getParticipantByCode(_.get(appData, "username"),authToken).then(res => {
...
<svg
              ...
              <li>
                <NavLink
                  to="/beneficiary/search"
                  className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${pathname.includes('beneficiary') &&
                    'bg-graydark dark:bg-meta-4'
                    }`}
                >
                <svg
