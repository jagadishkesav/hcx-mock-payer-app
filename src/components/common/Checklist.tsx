...
export default function Checklist(props: {
    ...
    const { scores, items, setItems, className, approval, setApproval, onApprove, onReject, claim, type } = props;

    const [sticky, setSticky] = useState(false);

    const settledValue = (claim.status === "Approved" || claim.status === "Rejected") ? claim.status : (type === "medical" ? claim.medical_info.status : claim.financial_info.status);
    ...
    return (
        <div className={`w-[350px] relative ${className}`} id="checklist">
            <div className={`z-10 w-[350px] p-1 ${sticky ? "fixed top-0 bottom-8 overflow-auto" : "absolute -translate-y-[20px]"}`}>
                {!settled ? (
                    <>
