...
async function generateKeycloakAdminToken(){
    return await generateToken(keycloakAdminUsername, keycloakAdminPassword);  
}
...
export async function generateTokenParticipant(username: any, password: any){
    ...
    return new Promise((resolve, reject) => {
        post(`${hcxUrl}/api/${apiVersion}/participant/auth/token/generate`, qs.stringify(body), headers)
        .then(function (response: { data: { access_token: any; }; }) {
            console.log("response", response);
            const accessToken = response.data.access_token;
            resolve(accessToken);
